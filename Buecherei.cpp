...
void ausgabeAusgeliehen(std::vector<Medium*>& medien);
...
int main()
{
    ...
    do  // Anzeige des Men�s
    {
        std::cout << std::endl
				  ...
				  << "(6): Alle ausgeliehenen Medien ausgeben" << std::endl
                  << "(7): Beenden" << std::endl;
...
void ausgabeAusgeliehen(std::vector<Medium*>& medien)
{
	{
		bool outputGegeben;
	    std::cout << "Ausgeliehene Medien in der Buecherei:" << std::endl;

	    for (std::vector<Medium*>::iterator it = medien.begin(); it != medien.end(); it++)
	    {
	    	if((*it)->getStatus() && !medien.empty())
	    	{
	        std::cout << "*************************************************************" << std::endl;
